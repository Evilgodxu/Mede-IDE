...
object LogCollector {
    ...
    private fun collectLogs(context: Context): String {
        ...
        if (fileLogs.isNotBlank()) {
            ...
            sb.append("(文件日志为空，目录: ").append(FileLogger.getLogDir()).append(")\n\n")
        }
    ...
    private fun createLogFile(context: Context, content: String): File {
        ...
        return File(logsDir, "mede_logs_$timestamp.log").apply { writeText(content) }
    }
    ...
    private fun shareLogFile(context: Context, file: File) {
        ...
        val shareIntent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "Mede Logs")
            putExtra(Intent.EXTRA_TEXT, "Attached are the application logs.")
            addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        }
        context.startActivity(Intent.createChooser(shareIntent, "发送日志").apply {
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
        })
    }
}
