...
// 云端模型设置内容
@Composable
fun SettingsCloudModel(chatViewModel: ChatViewModel?) {
...
private fun CloudModelCardContent(chatViewModel: ChatViewModel, chatState: ChatUiState) {
    var editingProfile by remember { mutableStateOf<com.medeide.jh.model.chat.CloudModelProfile?>(null) }
    ...
    Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.3f))) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            OutlinedButton(onClick = {
                ...
                editKey = ""
                editModel = ""
                editContextWindow = 184000
                editMaxTokens = 16000
                editMaxToolRounds = 200
                editingProfile = null
                editStep = 0
                testResult = null
                    ...
                    Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                            ...
                            OutlinedButton(onClick = {
                                ...
                                editMaxToolRounds = profile.maxToolRounds
            ...
            text = {
                Column(
                    ...
                    if (editStep == 0 && editingProfile == null) {
                        ...
                        // 工具调用最大轮次
                        OutlinedTextField(
                            value = editMaxToolRounds.toString(),
                            onValueChange = { v ->
                                editMaxToolRounds = v.filter { it.isDigit() }.take(4).toIntOrNull()
                                    ?: if (v.isEmpty()) 200 else editMaxToolRounds
                            },
                            label = { Text("工具调用最大轮次") },
                            placeholder = { Text("200") },
                            modifier = Modifier.fillMaxWidth(),
                            singleLine = true,
                            supportingText = { Text("超限后自动打断生成，防止死循环", style = MaterialTheme.typography.labelSmall) },
                        )
                            ...
                            OutlinedButton(
                                onClick = {
                                    ...
                                    CoroutineScope(Dispatchers.IO).launch {
                                        try {
                                            val client = com.medeide.jh.data.source.remote.CloudLLMClient(context)
                                            val config = com.medeide.jh.model.chat.CloudModelConfig(
                                                enabled = true,
                        ...
                        onClick = {
                            if (editEndpoint.isNotBlank() && editModel.isNotBlank()) {
                                if (editingProfile != null) {
                                    ...
                                    chatViewModel.addCloudProfile(editName, editEndpoint, editKey, editModel, editContextWindow, editMaxTokens, editMaxToolRounds)
                                }
