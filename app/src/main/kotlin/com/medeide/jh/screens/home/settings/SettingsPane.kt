        ...
        when (category) {
            SettingsCategory.General -> {
                if (state.isLoading) CategoryPlaceholder(stringResource(R.string.loading))
                    ...
                    SettingsThemeCard(state, onSetThemeMode, Modifier.fillMaxWidth())
                    SettingsLanguageCard(state, onSetLanguage, Modifier.fillMaxWidth())
                    SettingsGeneralCard(modifier = Modifier.fillMaxWidth())
                }
            }
            SettingsCategory.RoleDefinition -> SettingsRoleDefinition(
                rules = state.rules,
